...
@pytest.mark.integration
async def test_diagnostic_tools(mcp_server):
    """Test diagnostic tools."""
    async with Client(mcp_server) as client:
        # Test diagnose_pr_failures
        result = await client.call_tool(
            "diagnose_pr_failures", {"pr_number": "123", "job_name": "test-job"}
        )
        assert not result.is_error
        ...
        # Test diagnose_pr_build_status
        result = await client.call_tool(
            "diagnose_pr_build_status", {"pr_number": "123", "job_name": "test-job"}
        )
        assert not result.is_error
        ...
        # Test get_test_failures_from_artifacts
        result = await client.call_tool(
            "get_test_failures_from_artifacts",
            {"pr_number": "123", "job_name": "test-job"},
        )
        assert not result.is_error
...
@pytest.mark.integration
async def test_log_tools(mcp_server):
    """Test log-related tools."""
    async with Client(mcp_server) as client:
        # Test get_dci_job_logs with a dummy job ID (should return error)
        result = await client.call_tool("get_dci_job_logs", {"job_id": "dummy-job-id"})
        assert not result.is_error

        # The log tools might return non-JSON responses, so handle that gracefully
        if result.content and len(result.content) > 0:
            content_text = result.content[0].text
            try:
                ...
                assert (
                    ...
                    or "not found" in content_text.lower()
                )

        # Test get_dci_job_artifacts with a dummy job ID (should return error)
        result = await client.call_tool(
            "get_dci_job_artifacts", {"job_id": "dummy-job-id"}
        )
        assert not result.is_error
